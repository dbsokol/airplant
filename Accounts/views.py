...
def GenerateToken(request):
    ...
    # raise error if creating braintree customer fails:
    if not braintree_customer_result.is_success:
        raise('[Accounts.views.GetToken]: Braintree customer result [BraintreeResult:%s]' %braintree_customer_result.is_success)
    
    # generate token:
    braintree_client_token = gateway.client_token.generate({"customer_id" : braintree_token_kwargs['id'] })
    
    # TODO CHECK ERROR HANDLING FOR TOKEN CREATION
    # raise error if generating braintree token fails:
    # if not braintree_client_token.is_success:
    #     raise('[Accounts.views.GetToken]: Braintree client token result [BraintreeResult:%s]' %braintree_client_token.is_success)
    
    context = json.dumps({
        'status' : 0,
        'message' : 'Successfully built your payment token.',
...
def RedeemDiscounts(request):
    ...
    coupon_code = request.POST['coupon_code'].upper()
    discounts = {}
...
def Register(request):
    ...
    # raise error if braintree customer creation fails:
    if not braintree_customer_result.is_success:
        raise('[Accounts.views.Register]: Braintree error on customer create [%s]' %braintree_customer_result)
    ...
    # raise error if braintree payment method creation fails:
    if not braintree_payment_method_result.is_success:
        raise('[Accounts.views.Register]: Braintree error on payment method create [%s]' %braintree_payment_method_result)
    ...
    # raise error is braintree subscription creation fails:
    if not braintree_subscription_result.is_success:
        raise('[Accounts.views.Register]: Created braintree subscrption with [BraintreeResult:%s]' %braintree_subscription_result)
